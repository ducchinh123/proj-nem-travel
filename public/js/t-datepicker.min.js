...
; (function ($) {
	...
	var Defaults = {
		...
		titleMonths: ['Th 01', 'Th 02', 'Th 03', 'Th 04', 'Th 05', 'Th 06', 'Th 07', 'Th 08', 'Th 09', 'Th 10', 'Th 11', "Th 12"],
		titleMonthsLimitShow: 3,
		replaceTitleMonths: 'tháng', // Thg
		showDateTheme: 'dd-mm', // dd-mm-yy

		iconArrowTop: true,
		iconDate: '<i class="vicon vicon-check-in icon"></i><i class="vicon vicon-check-out icon"></i>',
		arrowPrev: '&#x276E;',
		arrowNext: '&#x276F;',
	...
	$.fn.tDatePicker = function (pr_el, options) {
		...
		function setThemeCheckDate(pr_title, pr_class, pr_data_utc, pr_input, pr_fm_input, pr_requied) {
			...
			if (settings.showdes) {
				var nightsstr = '';
				var dayOfWeek = pr_data_utc ? moment(pr_data_utc).format('dddd') : '';
				var desStr = '<p class="v_field__description">' + dayOfWeek + '</p>';
				if (pr_class.indexOf('check-in') != -1) {
					checkIn = moment(pr_data_utc);
				}
				if (pr_class.indexOf('check-out') != -1) {
					checkOut = moment(pr_data_utc);
					var nights = checkOut.diff(checkIn, "days");
					nights = nights ? nights : "";
					nightsstr = '<div class="nights"><span class="night">' + nights + '</span><i class="vicon vicon-free-night-stay icon"></i></div>';
				}
				return '<div class="t-dates col-xs-12 v_field t-date-' + pr_class + '">' + nightsstr +
					// setInfoTitle(pr_title, 't-date-info-title')
					settings.iconDate +
					'<label class="t-date-info-title">' + pr_title + '</label><div class="v_field__content"><p class="v_field__text">' +
					showThemeDate(pr_class, pr_data_utc) + '</p>' +
					desStr + '</div></div>' +
				...
				return '<div class="t-dates date-input-group t-date-' + pr_class + '">' +
					// setInfoTitle(pr_title, 't-date-info-title')
					settings.iconDate +
					'<label class="t-date-info-title">' + pr_title + '</label>' +
					'<span class="form-control date-input">' + showThemeDate(pr_class, pr_data_utc) +
					'</span></div>' +
					'<input type="text" class="t-input t-input-' + pr_class + '"' + ' value="' + pr_fm_input + '" name="' + pr_input + '"' + pr_requied + '>'
			}
		...
		function setThemeData(dataDays, dataUTCDate, pr_num, pr_el) {
			...
			for (var td = 0; td < getTD.length; td++) {
				...
				if (dataUTCDate[td] && $(window).outerWidth() < 768) {
					var formatMonth = `${new Date(dataUTCDate[td]).getFullYear()}-${new Date(dataUTCDate[td]).getMonth() + 1}`;
					$($(getTD[td]).parents('.t-table-wrap')).attr('month-of-year', formatMonth);
				}
			}
		}
		...
		function getStyleDays(pr_el, pr_data_utc) {
			...
			if ($(window).outerWidth() < 768) {
				$('.t-datepicker-header').remove();
				$('.t-datepicker-day').prepend('<div class="t-datepicker-header">' +
					'<div class="title">Chọn ngày</div>' +
					'<span class="t-datepicker-close"><img src="https://res.ivivu.com/flight/inbound/images/icon/ic_close.svg" /></span>' + '</div>');
				$('.t-table-wrap').first().css('padding-top', '40px');
					...
					if ($(this).parents('.t-check-out').hasClass('t-check-out') === true) {
						...
						setTimeout(function () {
							// $('.t-arrow-top').css({'display': 'none'})
								...
								settings.dateToHoldTable = data_utc_in;
							//}
						}, 10)
					}
			...
			if (settings.fnDataEvent != undefined && 'ontouchstart' in window === false) {
				...
				var dstyle1 = settings.fnDataStyle1;
				var dstyle2 = settings.fnDataStyle2;
				var dstyle3 = settings.fnDataStyle3;
				var mergeData = {};
				// return;
				// Find number limit month options.numCalendar 1,2,3 ...
				for (var cl = 0; cl < settings.numCalendar; cl++) {
					...
					for (var i = 0; i < toDayElement.length; i++) {
						...
						//if (dstyle1 && dstyle1[key] != undefined && getMonths === gMonth) {
						//	var cln = toDayElement[i].className;
						//	// cln = cln.replace('t-disabled', '');
						//	toDayElement[i].className = toDayElement[i].className.replace(' t-style-day-1', '') + ' t-style-day-1';
							 
						//	//toDayElement[i].setAttribute('t-date-title', check_num_10(getNum) + ' ' + settings.titleMonths[new Date(getDays).getMonth()] + ' - ' + data[key])
						//}
						//if (dstyle2 && dstyle2[key] != undefined && getMonths === gMonth) {
						//	var cln = toDayElement[i].className;
						//	// cln = cln.replace('t-disabled', '');
						//	toDayElement[i].className = toDayElement[i].className.replace(' t-style-day-2', '') + ' t-style-day-2';
						//	if ($(toDayElement[i]).parents('.t-datepicker-day').length != 0 && $(toDayElement[i]).parents('.t-datepicker-day').find('.t-date-style-note').length ==0) {
						//		$(toDayElement[i]).parents('.t-datepicker-day').append('<p class="t-date-style-note"><span class="style-bg-hot"></span> Giai đoạn giá siêu hot</p>')
						//	}
						//	//toDayElement[i].setAttribute('t-date-title', check_num_10(getNum) + ' ' + settings.titleMonths[new Date(getDays).getMonth()] + ' - ' + data[key])
						//}
						//if (dstyle3 && dstyle3[key] != undefined && getMonths === gMonth) {
						//	var cln = toDayElement[i].className;
						//	// cln = cln.replace('t-disabled', '');
						//	toDayElement[i].className = toDayElement[i].className.replace(' t-style-day-3', '') + ' t-style-day-3';
				...
				for (var cl = 0; cl < settings.numCalendar; cl++) {
					...
					for (var i = 0; i < toDayElement.length; i++) {
						...
						if ($(window).outerWidth() < 768) {
							if (data[key] != undefined && getMonths === gMonth) {
								var cln = toDayElement[i].className;
								var date = new Date(Number(toDayElement[i].getAttribute('data-t-date')));
								...
								if ('ontouchstart' in window === true) {
									var uniqueClass = 'unique-element-class';
									// $('.t-datepicker-notification').append('<p class="t-date-title mobile-special-day">' + check_num_10(getNum) + ' - ' + data[key]  + '</p>')
									var newElement = '<p class="' + uniqueClass + ' t-date-title mobile-special-day">' + check_num_10(getNum) + ' - ' + data[key] + '</p>';
									if (month == gMonth && year == gYear) {
										$('.t-table-wrap[month-of-year="' + `${gYear}-${month}` + '"]').append(newElement);
									}
								}
							}
						}
			...
			if (tArrow.length != 0) {
				...
				tArrow[0].onclick = function (e) {
					...
					if (Date.UTC(y, m) > limitPrevMonth) {
						m = m - 1;
						if ($(window).outerWidth() < 768) {
							$('.unique-element-class').remove();
						}
						// Nhận vào Elements [dates], date_utc = [1,2]
						setDaysInMonth(pr_el, Date.UTC(y, m));
						settings.dateToHoldTable = Date.UTC(y, m);
					}
				}
				// Next Calendar
		...
		function getTableCalendar(pr_el, pr_date_utc) {
			...
			if ($(pr_el).find('.t-datepicker-day').length !== 0) {
				...
				//if (settings.numCalendar == 2) {
				//	if (!settings.dateToHoldTable) {
				//		settings.dateToHoldTable = pr_date_utc;
				//	}
				//	setDaysInMonth($(pr_el).find('.t-dates').parent(), settings.dateToHoldTable)
				//}
				//else
	...
	$(document).on('click', '.t-datepicker-close', function () {
		setTimeout(function () {
